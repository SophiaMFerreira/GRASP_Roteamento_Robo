'''
Projeto GRASP: Nadine Vasconcellos e Sophia Ferreira
    ...
    while i < len(melhorRota) - 1:
        ...
        if (tuple(atual) in obstaculos) or (dx < 0 or dy < 0):
            ...
            for mov in (2, 1):
                ...
                if custoTeste < custoAtual:
                    melhorRota = rotaTeste[:];
                    melhorCusto = custoTeste;
                    i = max(1, i - 2);
                    break
            else:
                i += 1;
...
while (iP < condPlator) and (i < maxIteracao):
    ...
    if (custo < melhorCusto):
        ...
        melhorRota = rota[:];
        iP = 0;
        print(f"Iteração {i:10d} | Melhor Custo Parcial: ", melhorCusto);
    else:
        iP += 1;
    i += 1;
